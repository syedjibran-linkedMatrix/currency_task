...
class CurrencyDataFetcher:
    def __init__(self, days, num_currencies):
        self.days = days
        self.currencies = self.select_random_currencies(num_currencies)  # Select currencies here
    ...
    def fetch_data(self):
        ...
        all_data = {currency.value: [] for currency in self.currencies}  # Dictionary for selected currencies

        for i in range(self.days):
            date = (today - timedelta(days=i)).strftime('%Y-%m-%d')  # Get date for each day
            ...
            if data:  # Only store data if successfully fetched
                ...
                for currency in self.currencies:
                    value = eur_data.get(currency.value, None)  # Use None if currency not found
